jQuery(document).ready(function($) {
    console.log('hello1');
    function handleFormSubmissionlogin(form) {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res.status != "ok") {
                    ...
                    console.log(res.url);
                    // Remove unnecessary part from the URL
                    var newUrl = res.url.replace('/lam/user/login?process=login', '');
                    console.log(newUrl);
                    // You can use the new URL for further processing
    ...
    $(".lam-popup-login-button").on("click", function() {
        var form = $(this).closest(".lam-popup-login");
        handleFormSubmissionlogin(form);
        return false;
    });
});
